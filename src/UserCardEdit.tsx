...
const UserCardEdit: React.FC<UserCardEditProps> = ({ u }) => {
    ...
    try {
		...
		console.log(`http://localhost:4040/users/customers/${name}/${username}/${password}/${u.ssn}`);
    } catch (error) {
	...
	async function deleteUser() {
		try {
			console.log("delete");
			await axios.delete(
				`http://localhost:4040/users/customers/${u.ssn}`
			);
			setDeleted(true);
			console.log("deleted");
		} catch (error) {
	...
	return (
		<div className={deleted ? 'card w-96 bg-base-100 shadow-xl hidden' :  
        'card w-96 bg-base-100 shadow-xl'}>
			<form className='card-body' id="myForm">
				<h1 className='card-title'>Name: {u.name}</h1>
				<h2>SSN:</h2>
				<textarea className="textarea textarea-bordered" placeholder={u.ssn} value={ssn} onChange={onChange(setSsn)}></textarea>
				<h3>Username:</h3>
				<textarea className="textarea textarea-bordered" placeholder={u.username} value={username} onChange={onChange(setUsername)}></textarea>
				<p>Password:</p>
				<textarea className="textarea textarea-bordered" placeholder={u.password} value={password} onChange={onChange(setPassword)}></textarea>
				<div className='card-actions justify-end'>
				<button className='btn btn-primary' onClick={updateuser} type="button">
						Update Parameters
					</button>
