...
async function deleteBooking() {
	...
	return (
        <div>
		<div className={cancelled ? 'card w-96 bg-base-100 shadow-xl hidden' :  
        'card w-96 bg-base-100 shadow-xl'}>
			<div className='card-body'>
                <form className="p-0" id="myForm">
                <h1 className='card-title'>Room ID</h1>
                <textarea className="textarea textarea-bordered" placeholder={b.room_id.toString()} value={room_id}
								
								onChange={onChange(setRoom_id)}></textarea>
				<h2>CustomerID:</h2>
<textarea className="textarea textarea-bordered" placeholder={String(b.customer_id)} value={customer_id}
							
								onChange={onChange(setCustomer_id)}></textarea>
				<h3>Start Date:</h3>
				<input className="textarea textarea-bordered" type="date" value={moment(b.start_date).utc().format("YYYY-MM-DD")} 
               
                onChange={onChange(setStart_date)}></input>
				<p>End Date:</p>
				<input className="textarea textarea-bordered" type="date" value={moment(b.end_date).utc().format("YYYY-MM-DD")} 
                onChange={onChange(setEnd_date)}></input>
				<p>Checked in?:</p>
				<input className="textarea textarea-bordered" type="checkbox" checked={checked_in}
                onChange={()=>setChecked_in(!checked_in)}></input>
                
				<div className='card-actions justify-end'>
                    <button className='btn btn-primary' onClick={updateBooking} type='submit'>
                        Update Parameters
                    </button>
                    
					<button className='btn btn-error' onClick={deleteBooking} type="button">
						Delete
					</button>
